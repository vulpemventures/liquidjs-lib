...
export function equate(a: any, b: any, args?: any): void {
  ...
  if ('confidentialAddress' in b)
    t.deepStrictEqual(
      tryHex(a.confidentialAddress),
      tryHex(b.confidentialAddress),
      'Inequal *.confidentialAddress',
    );
}
...
export function preform(x: any): any {
  ...
  if (x.blindkey) x.blindkey = Buffer.from(x.blindkey, 'hex');
