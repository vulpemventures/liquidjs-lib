...
describe('liquidjs-lib (transactions with psetv2)', () => {
  ...
  it('can create (and broadcast via 3PBP) a confidential issuance and reissuance Transactions', async () => {
    ...
    const outputs = [
      new CreatorOutput(lbtc, 99999400, alice.payment.output, alice.payment.blindkey, 0),
    ...
    console.log('issuance owned inputs')
    ownedInputs.forEach(v => {
      console.log('index:', v.index)
      console.log('asset:', AssetHash.fromBytes(v.asset).hex)
      console.log('value:', v.value)
      console.log('assetBlinder:', v.assetBlindingFactor.toString('hex'))
      console.log('valueBlinder:', v.valueBlindingFactor.toString('hex'))
      console.log('')
    })

    console.log('issuance pset outputs')
    pset.outputs.forEach(v => {
      console.log('asset:', AssetHash.fromBytes(v.asset!).hex)
      console.log('value:', v.value)
      console.log('')
    })
    console.log('issuance pset out blinders')
    outputBlindingArgs.forEach(v => {
      console.log('index:',v.index)
      console.log('asset blinder:',v.assetBlinder.toString('hex'))
      console.log('value blinder:',v.valueBlinder.toString('hex'))
      console.log('')
    })
    ...
    const issuanceTx = signTransaction(pset, [alice.keys], Transaction.SIGHASH_ALL);
    ...
    const reissuanceOutputs = [
      new CreatorOutput(lbtc, 99998700, alice.payment.output, alice.payment.blindkey, 0),
    ...
    const reissuanceOwnedInputs = [
      {
        index: 0,
        asset: AssetHash.fromHex(lbtc).bytesWithoutPrefix,
        value: '99999400',
        assetBlindingFactor: outputBlindingArgs[0].assetBlinder,
        valueBlindingFactor: outputBlindingArgs[0].valueBlinder,
      },
      {
        index: 1,
        asset: pset.outputs[3].asset!,
        value: pset.outputs[3].value.toString(10),
        assetBlindingFactor: outputBlindingArgs[2].assetBlinder,
        valueBlindingFactor: outputBlindingArgs[2].valueBlinder,
      },
    ]

    console.log('reissuance owned inputs')
    reissuanceOwnedInputs.forEach(v => {
      console.log('index:', v.index)
      console.log('asset:', AssetHash.fromBytes(v.asset).hex)
      console.log('value:', v.value)
      console.log('assetBlinder:', v.assetBlindingFactor.toString('hex'))
    ...
    console.log('reissuance pset outputs')
    reissuancePset.outputs.forEach(v => {
      console.log('asset:', AssetHash.fromBytes(v.asset!).hex)
      console.log('value:', v.value)
      console.log('')
    })

    zkpGenerator = ZKPGenerator.fromOwnedInputs(reissuanceOwnedInputs)
    // const reissuanceBlindingArgs = await zkpGenerator.blindIssuances(reissuancePset, {
    ...
    console.log('reissuance pset out blinders')
    reissuanceOutputBlindingArgs.forEach(v => {
      console.log('index:',v.index)
      console.log('asset blinder:',v.assetBlinder.toString('hex'))
      console.log('value blinder:',v.valueBlinder.toString('hex'))
      console.log('')
    ...
    blinder = new PsetBlinder(reissuancePset, reissuanceOwnedInputs, zkpValidator, zkpGenerator)
    await blinder.blindLast({
      // issuanceBlindingArgs: reissuanceBlindingArgs,
      outputBlindingArgs: reissuanceOutputBlindingArgs,
    })

    const reissuanceTx = signTransaction(reissuancePset, [alice.keys, alice.keys], Transaction.SIGHASH_ALL);
