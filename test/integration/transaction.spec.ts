import { IssuanceBlindingKeys } from './../../types/types.d';
...
describe('liquidjs-lib (transactions with psbt)', () => {
  ...
  it('can create a 1-to-1 confidential Transaction (and broadcast via 3PBP) with unblinded issuance', async () => {
    ...
    psbt.finalizeAllInputs();
    const hex = psbt.extractTransaction().toHex();
    assert.doesNotReject(() => regtestUtils.broadcast(hex));
  });
...
const nonWitnessUtxoBuffer = Buffer.from(
