...
describe('liquidjs-lib (transactions with psbt)', () => {
  ...
  it('can create (and broadcast via 3PBP) a confidential Transaction with confidential AND unconfidential outputs', async () => {
    // these are { payment: Payment; keys: ECPair[] }
    const alice = createPayment('p2pkh', undefined, undefined, true); // confidential
    const bob = createPayment('p2pkh', undefined, undefined, false); // unconfidential
    ...
    const inputData1 = await getInputData(alice.payment, false, 'noredeem');
    {
      const {
        hash, // string of txid or Buffer of tx hash. (txid and hash are reverse order)
        index, // the output index of the txo you are spending
    ...
    const psbt = new liquid.Psbt({ network: regtest })
      ...
      .addOutput({
        ...
        value: satoshiToConfidentialValue(4999_3000),
      }) // Alice's change
      .addOutput({
        asset,
        nonce,
      ...
      .blindOutputsIndex(alice.blindingKeys, [aliceBlindingPubKey], [1])
