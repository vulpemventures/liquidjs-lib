...
import {
  ...
  ECPairFactory,
  Signer as ECPairSigner,
  SignerAsync as ECPairSignerAsync,
} from 'ecpair';
...
export class Psbt {
  ...
  finalizeInput(
    ...
    if (input.finalScriptWitness) return this; // already finalized
    const {
      script,
      isP2SH,
      isP2WSH,
  ...
  signInput(
    ...
    if (!keyPair || !keyPair.publicKey)
      throw new Error('Need Signer to sign input');
    const { hash, sighashType, needSchnoorSig } = getHashAndSighashType(
      this.data.inputs,
      inputIndex,
      keyPair.publicKey,
      this.__CACHE,
    ...
    if (!needSchnoorSig) {
      ...
      if (!keyPair.signSchnorr)
        throw new Error('Signer does not support Schnorr');
      const signature = keyPair.signSchnorr(hash);
      this.data.updateInput(inputIndex, { tapKeySig: signature });
    }
...
function canFinalize(
  ...
  switch (scriptType) {
    ...
    case 'taproot':
      if (!input.tapKeySig && !input.finalScriptWitness) return false;
      return true;
    default:
      return false;
  }
...
const isP2TR = (script: Buffer): boolean =>
  script && script[0] === 0x51 && script[1] === 0x20;
...
type FinalScriptsFunc = (
  ...
  isTaproot: boolean, // Is it Taproot script?
) => {
...
function getFinalScripts(
  ...
  return prepareFinalScripts(
    ...
    isP2WSH,
    isTaproot,
    input.sighashType === Transaction.SIGHASH_DEFAULT ||
      input.sighashType === Transaction.SIGHASH_ALL ||
      !input.sighashType
      ? input.tapKeySig
      : Buffer.concat([
          Buffer.of(
            input.sighashType ? input.sighashType : Transaction.SIGHASH_ALL,
          ),
          input.tapKeySig ? input.tapKeySig : Buffer.alloc(0),
        ]),
  );
}
...
function prepareFinalScripts(
  ...
  if (isSegwit) {
    ...
    finalScriptWitness = witnessStackToScriptWitness([taprootKeySig!]);
  } else {
    if (p2sh) {
      finalScriptSig = p2sh.input;
    } else {
...
function getHashAndSighashType(
  ...
  const isTaproot = isP2TR(script);
  if (!isTaproot) checkScriptForPubkey(pubkey, script, 'sign');
  return {
    hash,
    sighashType,
    needSchnoorSig: isTaproot,
  };
}
...
function getHashForSig(
  ...
  if (['p2sh-p2wsh', 'p2wsh'].indexOf(type) >= 0) {
    ...
    for (let i = 0; i < inputs.length; i++) {
      ...
      if (!witnessUtxo)
        throw new Error('need witnessUtxo to sign taproot input');
      prevoutScripts[i] = witnessUtxo.script;
      prevoutAssetsValues[i] = {
        asset: witnessUtxo.asset,
...
function getMeaningfulScript(
  ...
  type: 'p2sh' | 'p2wsh' | 'p2sh-p2wsh' | 'raw' | 'taproot';
} {
