 ...
 */
class Psbt {
    constructor(opts = {}, data = new bip174_liquid_1.Psbt(new PsbtTransaction())) {
        this.data = data;
        // set defaults
    ...
    static fromBuffer(buffer, opts = {}) {
        const psbtBase = bip174_liquid_1.Psbt.fromBuffer(buffer, transactionFromBuffer);
    ...
    get txOutputs() {
        return this.__CACHE.__TX.outs.map(output => {
            let address;
            try {
                address = (0, address_2.fromOutputScript)(output.script, this.opts.network);
            }
            catch (_) { }
            return {
                ...output,
    ...
    addIssuance(args, inputIndex) {
        ...
        const issuance = (0, issuance_1.newIssuance)(args.assetAmount, args.tokenAmount, args.precision, args.contract);
        const entropy = (0, issuance_1.generateEntropy)({ txHash: hash, vout: index }, issuance.assetEntropy);
        // add the issuance to the input.
        this.__CACHE.__TX.ins[inputIndex].issuance = issuance;
        const asset = Buffer.concat([issuancePrefix, (0, issuance_1.calculateAsset)(entropy)]);
        ...
        if (args.tokenAmount !== 0) {
            ...
            const token = (0, issuance_1.calculateReissuanceToken)(entropy, args.blindedIssuance);
    ...
    addReissuance(args) {
        ...
        const satsToReissue = (0, issuance_1.toConfidentialAssetAmount)(args.assetAmount, args.precision);
        ...
        const asset = Buffer.concat([issuancePrefix, (0, issuance_1.calculateAsset)(args.entropy)]);
        // send the asset amount to the asset address.
        this.addOutput({
            value: satsToReissue,
        ...
        const token = Buffer.concat([
            issuancePrefix,
            (0, issuance_1.calculateReissuanceToken)(args.entropy, args.blindedIssuance),
        ]);
        // send the token amount to the token address.
        this.addOutput({
            value: (0, issuance_1.toConfidentialTokenAmount)(args.tokenAmount, args.precision),
        ...
        if (updateData.witnessUtxo) {
            ...
            const asset = Buffer.isBuffer(witnessUtxo.asset)
                ...
                : witnessUtxo.asset.startsWith('0a') ||
                    ...
                    : Buffer.concat([
                        Buffer.alloc(1, 1),
                        (0, bufferutils_1.reverseBuffer)(Buffer.from(witnessUtxo.asset, 'hex')),
                    ]);
    ...
    blindOutputs(blindingDataLike, blindingPubkeys, opts) {
        return this.rawBlindOutputs(blindingDataLike, blindingPubkeys, undefined, undefined, opts);
    }
    blindOutputsByIndex(inputsBlindingData, outputsBlindingPubKeys, issuancesBlindingKeys, opts) {
        const blindingPrivKeysArgs = range(this.__CACHE.__TX.ins.length).map((inputIndex) => inputsBlindingData.get(inputIndex));
        const blindingPrivKeysIssuancesArgs = issuancesBlindingKeys
            ? range(this.__CACHE.__TX.ins.length).map((inputIndex) => issuancesBlindingKeys.get(inputIndex))
            : [];
        const outputIndexes = [];
        ...
        return this.rawBlindOutputs(blindingPrivKeysArgs, blindingPublicKey, blindingPrivKeysIssuancesArgs, outputIndexes, opts);
    }
    ...
    searchInputIndexForIssuance(inputIndex) {
        if (inputIndex && !this.data.inputs[inputIndex]) {
            ...
            if (this.__CACHE.__TX.ins.filter(i => !i.issuance).length === 0)
                throw new Error('transaction needs at least one input without issuance data.');
    ...
    unblindInputsToIssuanceBlindingData(issuanceBlindingPrivKeys = []) {
        ...
        for (const input of this.__CACHE.__TX.ins) {
            if (input.issuance) {
                const isConfidentialIssuance = issuanceBlindingPrivKeys && issuanceBlindingPrivKeys[inputIndex]
                    ? true
                    : false;
                ...
                const assetBlindingData = {
                    ...
                    valueBlindingFactor: isConfidentialIssuance ? randomBytes() : transaction_1.ZERO,
                };
                pseudoBlindingDataFromIssuances.push(assetBlindingData);
                if (!(0, issuance_1.isReissuance)(input.issuance) && (0, issuance_1.hasTokenAmount)(input.issuance)) {
                    const token = (0, issuance_1.calculateReissuanceToken)(entropy, isConfidentialIssuance);
    ...
    async blindInputs(blindingData, issuanceBlindingPrivKeys = []) {
        ...
        function getBlindingFactors(asset) {
            ...
            throw new Error('no blinding factors generated for pseudo issuance inputs');
        }
        ...
        for (const input of this.__CACHE.__TX.ins) {
            if (input.issuance) {
                ...
                const assetCommitment = await confidential.assetCommitment(blindingFactorsAsset.asset, blindingFactorsAsset.assetBlindingFactor);
                const valueCommitment = await confidential.valueCommitment(blindingFactorsAsset.value, assetCommitment, blindingFactorsAsset.valueBlindingFactor);
                ...
                if (!assetBlindingPrivateKey) {
                    throw new Error(`missing asset blinding private key for issuance #${inputIndex}`);
                }
                const issuanceRangeProof = await confidential.rangeProof(blindingFactorsAsset.value, assetBlindingPrivateKey, blindingFactorsAsset.asset, blindingFactorsAsset.assetBlindingFactor, blindingFactorsAsset.valueBlindingFactor, valueCommitment, Buffer.alloc(0), '1', 0, 52);
                this.__CACHE.__TX.ins[inputIndex].issuanceRangeProof = issuanceRangeProof;
                this.__CACHE.__TX.ins[inputIndex].issuance.assetAmount = valueCommitment;
                if (!(0, issuance_1.isReissuance)(input.issuance) && (0, issuance_1.hasTokenAmount)(input.issuance)) {
                    const token = (0, issuance_1.calculateReissuanceToken)(entropy, true);
                    const blindingFactorsToken = getBlindingFactors(token);
                    const issuedTokenCommitment = await confidential.assetCommitment(token, blindingFactorsToken.assetBlindingFactor);
                    const tokenValueCommitment = await confidential.valueCommitment(blindingFactorsToken.value, issuedTokenCommitment, blindingFactorsToken.valueBlindingFactor);
                    if (!issuanceBlindingPrivKeys[inputIndex].tokenKey) {
                        throw new Error('you must specify tokenKey in order to blind the token issuance');
                    }
                    const inflationRangeProof = await confidential.rangeProof(blindingFactorsToken.value, issuanceBlindingPrivKeys[inputIndex].tokenKey, token, blindingFactorsToken.assetBlindingFactor, blindingFactorsToken.valueBlindingFactor, tokenValueCommitment, Buffer.alloc(0), '1', 0, 52);
                    this.__CACHE.__TX.ins[inputIndex].inflationRangeProof = inflationRangeProof;
                    this.__CACHE.__TX.ins[inputIndex].issuance.tokenAmount = tokenValueCommitment;
                }
            }
            inputIndex++;
        }
    ...
    async blindOutputsRaw(blindingData, blindingPubkeys, outputIndexes, opts) {
        ...
        const outputsBlindingData = await computeOutputsBlindingData(blindingData, outputsData);
        ...
        for (const outputIndex of outputIndexes) {
            ...
            const outputNonce = ecpair_1.ECPair.fromPrivateKey(ephemeralPrivKey).publicKey;
            const outputBlindingData = outputsBlindingData[indexInArray];
            // commitments
            const assetCommitment = await confidential.assetCommitment(outputBlindingData.asset, outputBlindingData.assetBlindingFactor);
            const valueCommitment = await confidential.valueCommitment(outputBlindingData.value, assetCommitment, outputBlindingData.valueBlindingFactor);
            // proofs
            const rangeProof = await confidential.rangeProofWithNonceHash(outputBlindingData.value, blindingPubkeys[indexInArray], ephemeralPrivKey, outputBlindingData.asset, outputBlindingData.assetBlindingFactor, outputBlindingData.valueBlindingFactor, valueCommitment, this.__CACHE.__TX.outs[outputIndex].script);
            const surjectionProof = await confidential.surjectionProof(outputBlindingData.asset, outputBlindingData.assetBlindingFactor, blindingData.map(({ asset }) => asset), blindingData.map(({ assetBlindingFactor }) => assetBlindingFactor), randomSeed);
    ...
    async rawBlindOutputs(blindingDataLike, blindingPubkeys, issuanceBlindingPrivKeys = [], outputIndexes, opts) {
        if (this.data.inputs.some((v) => !v.nonWitnessUtxo && !v.witnessUtxo))
            throw new Error('All inputs must contain a non witness utxo or a witness utxo');
        if (this.__CACHE.__TX.ins.length !== blindingDataLike.length) {
            throw new Error('blindingDataLike length does not match the number of inputs (undefined for unconfidential utxo)');
        }
        if (!outputIndexes) {
            outputIndexes = [];
        ...
        if (outputIndexes.length !== blindingPubkeys.length)
            throw new Error('not enough blinding public keys to blind the requested outputs');
        ...
        const inputsBlindingData = await Promise.all(blindingDataLike.map((data, i) => toBlindingData(data, witnesses[i])));
        const pseudoInputsBlindingData = this.unblindInputsToIssuanceBlindingData(issuanceBlindingPrivKeys);
        const totalBlindingData = inputsBlindingData.concat(pseudoInputsBlindingData);
        await this.blindOutputsRaw(totalBlindingData, blindingPubkeys, outputIndexes, opts);
...
function checkTxEmpty(tx) {
    const isEmpty = tx.ins.every(input => input.script && input.script.length === 0);
    if (!isEmpty) {
...
function getHashAndSighashType(inputs, inputIndex, pubkey, cache, sighashTypes) {
    ...
    const { hash, sighashType, script } = getHashForSig(inputIndex, inputs[inputIndex], cache, false, sighashTypes);
...
async function computeOutputsBlindingData(inputsBlindingData, outputsData) {
    ...
    const [lastOutputValue, lastOutputAsset] = outputsData[outputsData.length - 1];
    ...
    // asset blinders
    const inputsAssetBlinders = inputsBlindingData.map(({ assetBlindingFactor }) => assetBlindingFactor);
    ...
    // value blinders
    const inputsAmountBlinders = inputsBlindingData.map(({ valueBlindingFactor }) => valueBlindingFactor);
    ...
    const finalAmountBlinder = await confidential.valueBlindingFactor(inputsValues, outputsValues, inputsAssetBlinders, outputsAssetBlinders, inputsAmountBlinders, outputsAmountBlinders);
...
async function toBlindingData(blindDataLike, witnessUtxo) {
    if (!blindDataLike) {
        if (!witnessUtxo)
            throw new Error('need witnessUtxo');
...
function validateAddIssuanceArgs(args) {
    ...
    if (args.tokenAddress) {
        if ((0, address_1.isConfidential)(args.assetAddress) !== (0, address_1.isConfidential)(args.tokenAddress)) {
            throw new Error('tokenAddress and assetAddress are not of the same type (confidential or unconfidential).');
        }
    }
}
...
function isPrevoutConfidential(args) {
    ...
    if (args.nonWitnessUtxo &&
        isConfidentialWitnessUtxo(transaction_1.Transaction.fromBuffer(args.nonWitnessUtxo).outs[args.tokenPrevout.vout])) {
        return true;
    }
    return false;
}
function isConfidentialWitnessUtxo(witnessUtxo) {
    return (witnessUtxo.rangeProof !== undefined &&
