...
function toBlech32(data, pubkey, prefix, witnessVersion) {
    return blech32_1.Blech32Address.from(data.slice(2).toString('hex'), pubkey.toString('hex'), prefix, witnessVersion).address;
}
...
function toOutputScript(address, network) {
    ...
    if (decodedBase58) {
        ...
        if (decodedBech32) {
            ...
            else if (decodedBech32.version >= FUTURE_SEGWIT_MIN_VERSION &&
            ...
            if (decodedConfidential) {
                return toOutputScript(decodedConfidential.unconfidentialAddress, network);
            }
        }
    }
...
function decodeBlech32(address) {
    ...
    switch (blech32addr.data.length - 2) {
        ...
        default:
            throw new Error(`invalid blech32 program length: ${blech32addr.data.length - 2}`);
    }
}
