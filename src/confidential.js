...
class Confidential {
  constructor(secp256k1Promise) {
    this.secp256k1Promise = secp256k1Promise;
  }
  async nonceHash(pubkey, privkey) {
    const { ecdh } = await this.secp256k1Promise;
    return crypto.sha256(ecdh(pubkey, privkey));
  }
  async valueBlindingFactor(
    inValues,
    outValues,
    inGenerators,
    outGenerators,
    ...
    const { pedersen } = await this.secp256k1Promise;
  ...
  async valueCommitment(value, gen, factor) {
    const { generator, pedersen } = await this.secp256k1Promise;
  ...
  async assetCommitment(asset, factor) {
    const { generator } = await this.secp256k1Promise;
    const gen = generator.generateBlinded(asset, factor);
    return generator.serialize(gen);
  }
  async unblindOutputWithKey(out, blindingPrivKey) {
    const nonce = await this.nonceHash(out.nonce, blindingPrivKey);
    return this.unblindOutputWithNonce(out, nonce);
  }
  async unblindOutputWithNonce(out, nonce) {
  ...
  async rangeProofInfo(proof) {
    const { rangeproof } = await this.secp256k1Promise;
  ...
  async rangeProofWithNonceHash(
    ...
    const nonce = await this.nonceHash(blindingPubkey, ephemeralPrivkey);
  ...
  async rangeProofVerify(valueCommit, assetCommit, proof, script) {
    const { generator, pedersen, rangeproof } = await this.secp256k1Promise;
  ...
  async surjectionProof(
    ...
    const { generator, surjectionproof } = await this.secp256k1Promise;
  ...
  async blindValueProof(value, valueCommit, assetCommit, valueBlinder, opts) {
    const { generator, pedersen, rangeproof } = await this.secp256k1Promise;
    const gen = generator.parse(assetCommit);
    const commit = pedersen.commitParse(valueCommit);
  ...
  async blindAssetProof(asset, assetCommit, assetBlinder) {
    const { generator, surjectionproof } = await this.secp256k1Promise;
  ...
  async assetBlindProofVerify(asset, assetCommit, proof) {
    const { generator, surjectionproof } = await this.secp256k1Promise;
...
class ZKPValidator {
  ...
  async verifyValueRangeProof(valueCommit, assetCommit, proof, script) {
    try {
      ...
      return await confidential.rangeProofVerify(
        valueCommit,
        assetCommit,
        proof,
        script,
  ...
  async verifyAssetSurjectionProof(
    ...
    try {
      const confidential = new Confidential(this.zkpLib);
      return await confidential.surjectionProofVerify(
        inAssets,
        inAssetBlinders,
        outAsset,
        outAssetBlinder,
  ...
  async verifyBlindAssetProof(asset, assetCommit, proof) {
    try {
      const confidential = new Confidential(this.zkpLib);
      return await confidential.assetBlindProofVerify(
        asset,
        assetCommit,
        proof,
      );
...
class ZKPGenerator {
  ...
  static fromOwnedInputs(ownedInputs, zkpLib) {
    const bg = new ZKPGenerator(zkpLib);
    bg.ownedInputs = ownedInputs;
    return bg;
  }
  static fromInBlindingKeys(inBlindingKeys, zkpLib) {
    const bg = new ZKPGenerator(zkpLib);
    bg.inBlindingKeys = inBlindingKeys;
    return bg;
  }
  static fromMasterBlindingKey(masterKey, zkpLib) {
    const bg = new ZKPGenerator(zkpLib);
  ...
  async computeAndAddToScalarOffset(scalar, value, assetBlinder, valueBlinder) {
    ...
    const { ec } = await this.secp256k1Promise;
  ...
  async lastValueCommitment(value, asset, blinder) {
    const confidential = new Confidential(this.secp256k1Promise);
    return confidential.valueCommitment(value, asset, blinder);
  }
  async lastBlindValueProof(value, valueCommit, assetCommit, blinder) {
    const confidential = new Confidential(this.secp256k1Promise);
    return confidential.blindValueProof(
      value,
      valueCommit,
      assetCommit,
      blinder,
  ...
  async lastValueRangeProof(
    ...
    const confidential = new Confidential(this.secp256k1Promise);
    return confidential.rangeProof(
      value,
      nonce,
      asset,
      assetBlinder,
    ...
    return Promise.all(
      Object.entries(blindingKeysByIndex).map(async ([i, key]) => {
        ...
        if (input.issuanceValue > 0) {
          ...
          const assetCommit = await confidential.assetCommitment(
            asset,
            transaction_1.ZERO,
          );
          const valueCommit = await confidential.valueCommitment(
            value,
            assetCommit,
            blinder,
          );
          const blindproof = await confidential.blindValueProof(
            value,
            valueCommit,
            assetCommit,
            blinder,
        ...
        if (input.issuanceInflationKeys > 0) {
          ...
          const tokenCommit = await confidential.valueCommitment(
            token,
            assetCommit,
            blinder,
          );
  ...
  async blindOutputs(pset, keysGenerator, outIndexes, blindedIssuances) {
    ...
    return Promise.all(
      outputIndexes.map(async (i) => {
        ...
        const ecdhNonce = await confidential.nonceHash(
        ...
        const valueBlindProof = await confidential.blindValueProof(
          value,
          valueCommit,
          assetCommit,
          valueBlinder,
        );
        const assetBlindProof = await confidential.blindAssetProof(
          output.asset,
          assetCommit,
          assetBlinder,
        );
  ...
  async unblindUtxo(out) {
    ...
    for (const key of keys) {
      try {
        const confidential = new Confidential(this.secp256k1Promise);
        const revealed = await confidential.unblindOutputWithKey(out, key);
        return {
          index: 0,
